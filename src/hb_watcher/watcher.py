...
def _default_config_path() -> str:
    """Return the default installed config file path."""
    installed = Path(f"/etc/{CONFIG_DIR_NAME}/{CONFIG_FILE_NAME}")
...
class HbWatcher:
    ...
    def is_in_maintenance(self, job, windows):
        for w in windows:
            if (
                self.matches_filter(w.get("hostname_filter"), job.get("hostname"))
                and self.matches_filter(w.get("app_name_filter"), job.get("app_name"))
                and self.matches_filter(w.get("port_filter"), job.get("port"))
                and self.matches_filter(w.get("task_filter"), job.get("task"))
                and self.matches_filter(w.get("version_filter"), job.get("version"))
            ):
                return True
        return False
    ...
    def update_django(self, updates, failed_delivery):
        ...
        requests.post(
            f"{self.api_url}/bulk_transition/",
            json=payload,
            auth=(self.api_user, self.api_pass),
            headers=headers,
            timeout=self.timeout,
    ...
    def build_actions_header(self, tokens_list):
        ...
        return (
            f"http, Acknowledge All, {ack_url}, method=POST, clear=true; "
            f"http, Snooze 1 hr, {snooze_url}, method=POST, clear=true"
        )
                ...
                updates.append(
                    {
                        ...
                        "message": msg or "Auto-transition.",
                    }
                )
        ...
        while True:
            try:
                ...
                if any(messages.values()):
                    ...
                    delivery_failed = not self.dispatch_notification(
                        "Heartbeat Update", body, prio, tags, actions
                    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Heartbeat Watcher Daemon")
    parser.add_argument(
        "--config",
        default=None,
        help=f"Path to config JSON (default: /etc/{CONFIG_DIR_NAME}/{CONFIG_FILE_NAME})",
    )
    args = parser.parse_args()
    HbWatcher(config_path=args.config).run_forever()
